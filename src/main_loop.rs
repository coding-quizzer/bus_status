...
pub fn main_loop(
    ...
    let rejected_passengers_pointer: Arc<Mutex<Vec<Passenger>>> = Arc::new(Mutex::new(Vec::new()));
    ...
    let receive_in_station_channels = receive_in_station_channels.into_iter().map(Some).collect();
